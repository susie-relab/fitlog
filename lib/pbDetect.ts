...
type PriorFields = Pick<Activity, 'exercise_type' | 'distance_km' | 'pace_min_km' | 'duration_minutes'>;
type NewFields = Pick<Activity, 'exercise_type' | 'distance_km' | 'pace_min_km' | 'duration_minutes'>;
...
export function detectAutoPBs(activity: NewFields, prior: PriorFields[]): string[] {
  ...
  if (activity.distance_km && activity.pace_min_km) {
    const bucket = DISTANCE_PB_KM.find(km => Math.abs(activity.distance_km! - km) / km < 0.02);
    if (bucket) {
      const priorPaces = prior
        .filter(a => a.distance_km && a.pace_min_km && Math.abs(a.distance_km - bucket) / bucket < 0.02)
        .map(a => a.pace_min_km!);
  ...
  // Longest distance for this exercise type.
  ...
  // Longest duration for this exercise type.
  ...
  if (activity.pace_min_km) {
    const priorPaces = sameType.filter(a => a.pace_min_km).map(a => a.pace_min_km!);
    if (priorPaces.length > 0 && activity.pace_min_km < Math.min(...priorPaces)) {
